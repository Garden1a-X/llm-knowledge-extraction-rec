#!/usr/bin/env python3
"""
Train MMGCN (Multi-modal Graph Convolution Network) model.

Usage:
    python baselines/train_mmgcn.py \
        --data_dir data/recbole/ml-1m \
        --visual_features data/recbole/ml-1m/visual_features.npy \
        --output_dir outputs/mmgcn \
        --seed 42
"""
...
def evaluate(model, dataloader, device, k=10, n_items=None, train_data=None, val_data=None):
    ...
    # Pre-compute all embeddings with GCN propagation
...
def main():
    ...
    # Load visual features and build adjacency matrix
    model.load_visual_features(visual_features)
    model.adj_matrix = model.build_adjacency_matrix(train_data, device)
