#!/usr/bin/env python3
"""
Train MMGCN using our own data split (same as our model).
...
def evaluate_uni100(model, test_data, train_pos, val_pos, n_items, device, k=10):
    ...
    # Pre-compute all embeddings with GCN
    print("  Pre-computing embeddings with GCN...")
    with torch.no_grad():
...
def main():
    ...
    print("="*60)
    print("MMGCN with Our Data Split")
    print("="*60)
    ...
    train_data_for_adj = [(u, i, 1.0) for u, i in train_interactions]
    model.adj_matrix = model.build_adjacency_matrix(train_data_for_adj, device)
    ...
    for epoch in range(args.epochs):
        ...
        if val_metrics['ndcg@10'] > best_val_ndcg:
            ...
            best_epoch = epoch + 1
            patience = 0
            torch.save(model.state_dict(), 'outputs/beauty/mmgcn_our_split_best.pth')
        else:
            patience += 1
    ...
    model.load_state_dict(torch.load('outputs/beauty/mmgcn_our_split_best.pth'))
    ...
    print()
    print("="*60)
    print("Test Results (MMGCN with Our Split)")
    print("="*60)
