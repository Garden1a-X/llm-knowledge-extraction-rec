#!/usr/bin/env python3
"""
Summarize LightGCN baseline results on Beauty dataset (5 seeds).
...
def extract_recbole_metrics_from_log(log_file: Path) -> Dict[str, float]:
    """
    Extract test metrics from RecBole LightGCN log file.
...
def process_lightgcn_trials(trial_dir: Path) -> Dict:
    """
    Process all 5 LightGCN trials and generate summary.

    Args:
        trial_dir: Directory containing trial logs (e.g., lightgcn_20260117_143419)
    """
    # Find all log files
    ...
    # Extract timestamp from directory name (lightgcn_20260117_143419)
...
def main():
    parser = argparse.ArgumentParser(description='Summarize LightGCN baseline trial results on Beauty')
    parser.add_argument('trial_dir', type=str, nargs='?', default=None,
                       help='Path to specific trial directory (e.g., outputs/beauty_baselines/lightgcn_20260117_143419)')
    ...
    if args.trial_dir:
        ...
        # Auto-detect: look for most recent LightGCN trial
        script_dir = Path(__file__).parent.parent
        base_dir = script_dir / 'outputs' / 'beauty_baselines'

        if not base_dir.exists():
            print(f"Error: Directory not found: {base_dir}")
            print("Please run ./scripts/run_beauty_lightgcn_5trials.sh first")
            return

        # Find most recent LightGCN trial
        trial_dirs = sorted(base_dir.glob('lightgcn_*'))
        if not trial_dirs:
            print(f"Error: No LightGCN trials found in {base_dir}")
            print("Please run ./scripts/run_beauty_lightgcn_5trials.sh first")
            return
    ...
    print("="*70)
    print("Summarizing LightGCN Baseline Results (Beauty)")
    print("="*70)
    print(f"Processing: {trial_dir}")
    print()

    try:
        summary = process_lightgcn_trials(trial_dir)

        # Save summary
        output_file = trial_dir.parent / f"lightgcn_amazon_beauty_summary_5trials.json"
