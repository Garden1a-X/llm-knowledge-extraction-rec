#!/usr/bin/env python3
"""
Convert Beauty user interests to RecBole .kg format.

Generates:
- amazon-beauty.user.kg: User interest knowledge graph
- amazon-beauty.kg: Merged item + user knowledge graph

Usage:
    python scripts/convert_beauty_user_interests_to_kg.py \
        --input results/beauty_user_interests.json \
        --output_user data/recbole/amazon-beauty/amazon-beauty.user.kg \
        --output_merged data/recbole/amazon-beauty/amazon-beauty.kg \
        --item_kg data/recbole/amazon-beauty/amazon-beauty.item.kg
"""
    ...
    for result in results:
        ...
        if long_term:
            ...
            for interest in long_term:
                # Only use entity, not relation (relation becomes "long_term_interest")
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert Beauty user interests to RecBole KG format'
    )

    parser.add_argument('--input', type=str,
                        default='results/beauty_user_interests.json',
                        help='Input user interests JSON')
    parser.add_argument('--output_user', type=str,
                        default='data/recbole/amazon-beauty/amazon-beauty.user.kg',
                        help='Output user KG file (.user.kg)')
    parser.add_argument('--output_merged', type=str,
                        default='data/recbole/amazon-beauty/amazon-beauty.kg',
                        help='Output merged KG file (.kg)')
    parser.add_argument('--item_kg', type=str,
    ...
    if verbose:
        print("="*70)
        print("BEAUTY USER INTEREST TO RECBOLE KG CONVERSION")
        print("="*70)
        print()
