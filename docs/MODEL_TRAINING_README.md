...
| 配置 | NDCG@10 | Recall@10 | 训练时间（300 epochs）|
|------|---------|-----------|----------------------|
| Ours-Full | ~0.12-0.15 | ~0.23-0.28 | ~10分钟（单卡A800）|
| Ours w/o Contrast | ~0.11-0.14 | ~0.22-0.26 | ~8分钟 |
| Ours w/o Mask | ~0.11-0.14 | ~0.21-0.25 | ~10分钟 |
| Ours KG-only | ~0.10-0.13 | ~0.20-0.24 | ~8分钟 |
...
1. **Baseline对比**：实现BPR, LightGCN等baseline
2. **超参数搜索**：GridSearch或Bayesian Optimization
3. **可视化分析**：
   - Mask权重分布
   - Embedding t-SNE可视化
   - Attention权重可视化
